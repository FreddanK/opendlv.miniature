...
KalmanFilter::KalmanFilter(Eigen::Vector3d initState, Eigen::Matrix3d Q, Eigen::Matrix2d R)
  : m_stateEstimate(initState)
  , m_Q(Q)
  , m_R(R)
  , m_P(Eigen::MatrixXd::Identity(3, 3))
...
KalmanFilter::KalmanFilter(Eigen::Vector3d initState, Eigen::Matrix3d Q, Eigen::Matrix2d R, Eigen::Matrix3d P_0)
  : m_stateEstimate(initState)
  , m_Q(Q)
  , m_R(R)
  , m_P(P_0)
...
void KalmanFilter::setR(Eigen::Matrix2d R)
{
  m_R = R;
}


Eigen::Vector3d KalmanFilter::updateEstimate(double sensorX, double sensorY,
  double speedLeftWheel, double speedRightWheel, double dt)
{
  ...
  Eigen::MatrixXd K(3, 2); // Kalman gain
  Eigen::Vector2d y(sensorX, sensorY);
...
Eigen::Vector2d KalmanFilter::sensorFunction()
{
  // Sensor function describing the behavior of the sensors.

  return Eigen::Vector2d(m_stateEstimate(0), m_stateEstimate(1));
}
...
Eigen::Matrix2d KalmanFilter::getSensorJacobian()
{
